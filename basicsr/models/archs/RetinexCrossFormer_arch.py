...
class Cross_Attention_MSA(nn.Module):
    ...
    def forward(self, x_in, illu_fea_trans, illu_map_trans):
        ...
        # 修改为 emb 之前 * x_q
        x_k = illu_fea_trans.reshape(b, h * w, c) * x_q
        x_v = illu_map_trans.reshape(b, h * w, c) * x_q
        ...
        out = out.reshape(b, h * w, self.num_heads * self.dim_head)

        # output projection
        # print("out: ", out.size())
        out = self.proj(out).view(x_in.shape)

        # print("q: ", q.size())
        #### 修改为 k_inp
        out_p = self.pos_emb(k_inp.reshape(b, h, w, c).permute(
...
class IGAB(nn.Module):
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                Cross_Attention_MSA(dim=dim, dim_head=dim_head, heads=heads),
    ...
    def forward(self, x, illu_fea, illu_map):
        ...
        for (attn, ff) in self.blocks:
            ...
            x = attn(x, illu_fea_trans=illu_fea.permute(0, 2, 3, 1), illu_map_trans=illu_map.permute(0, 2, 3, 1)) + x
            x = ff(x) + x
...
class Denoiser(nn.Module):
    def __init__(self, in_dim=3, out_dim=3, dim=31, n_feat=31, level=2, num_blocks=[2, 4, 4]):
        ...
        self.map_up = nn.Conv2d(in_dim, self.dim, kernel_size=1, bias=False)
        ...
        for i in range(level):
            self.encoder_layers.append(nn.ModuleList([
                # Illumination_Estimator(n_fea_middle=dim_level * 2, n_fea_in=dim_level + 1, n_fea_out=dim_level * 2),
                IGAB(
                ...
                nn.Conv2d(dim_level, dim_level * 2, 4, 2, 1, bias=False),  # IlluMapDownsample
            ]))
            dim_level *= 2
    ...
    def forward(self, x, illu_fea, illu_map):
        ...
        illu_map = self.map_up(illu_map)

        # Encoder
        fea_encoder = []
        illu_fea_list = []
        illu_map_list = []
        for (IGAB, FeaDownSample, IlluFeaDownsample, IlluMapDownsample) in self.encoder_layers:
        ...
        for i, (FeaUpSample, Fution, LeWinBlcok) in enumerate(self.decoder_layers):
            ...
            illu_map = illu_map_list[self.level - 1 - i]
...
if __name__ == '__main__':
    ...
    model = RetinexCrossFormer(stage=1,n_feat=40,num_blocks=[1,2,2]).cuda()
    print(model)
