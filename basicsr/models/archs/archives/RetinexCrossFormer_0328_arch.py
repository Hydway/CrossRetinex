...
class RGB_HVI(nn.Module):
    def __init__(self, input_dim=3, output_dim=3):
        ...
        # 初始化一个固定的转换矩阵（RGB 到 YUV）
        initial_matrix = torch.tensor([[0.299, 0.587, 0.114],
                                       [-0.14713, -0.28886, 0.436],
                                       [0.615, -0.51499, -0.10001]], dtype=torch.float32).to(self.device)
        self.initial_matrix = initial_matrix  # 保存初始矩阵

        # 初始化旋转角度参数
        self.rotation_angle_y = nn.Parameter(torch.tensor(0.0, dtype=torch.float32).to(self.device))  # 围绕 Y 轴的旋转角度
        self.rotation_angle_z = nn.Parameter(torch.tensor(0.0, dtype=torch.float32).to(self.device))  # 围绕 Z 轴的旋转角度

    def get_rotation_matrix(self):
        """
        生成围绕 Y 轴和 Z 轴的旋转矩阵。
        """
        cos_y = torch.cos(self.rotation_angle_y)
        sin_y = torch.sin(self.rotation_angle_y)
        cos_z = torch.cos(self.rotation_angle_z)
        sin_z = torch.sin(self.rotation_angle_z)
    ...
    def HVIT(self, img):
        ...
        img_flat = img.view(batch_size, 3, -1)  # 展平图像 (batch_size, 3, height * width)

        # 获取旋转矩阵
        rotation_matrix = self.get_rotation_matrix()

        # 使用旋转矩阵将初始矩阵旋转
        self.transform_matrix = torch.matmul(rotation_matrix, self.initial_matrix)

        # 使用旋转后的转换矩阵将 RGB 转换到新色彩空间
        transformed_flat = torch.matmul(self.transform_matrix, img_flat)  # 转换后的扁平图像 (batch_size, output_dim, height * width)

        transformed = transformed_flat.view(batch_size, self.output_dim, height, width)  # 恢复维度 (batch_size, output_dim, height, width)
        return transformed
    ...
    def PHVIT(self, img):
        ...
        """
        batch_size, _, height, width = img.shape
        img_flat = img.view(batch_size, self.output_dim, -1)  # 扁平化图像 (batch_size, output_dim, height * width)

        # 获取旋转矩阵
        rotation_matrix = self.get_rotation_matrix()

        # 使用旋转矩阵将初始矩阵旋转
        self.inverse_transform_matrix = torch.inverse(torch.matmul(rotation_matrix, self.initial_matrix))

        # 使用逆变换矩阵将新色彩空间转换回 RGB
        inverse_transformed_flat = torch.matmul(self.inverse_transform_matrix, img_flat)  # 转换后的扁平图像 (batch_size, input_dim, height * width)

        inverse_transformed = inverse_transformed_flat.view(batch_size, self.input_dim, height, width)  # 恢复维度 (batch_size, input_dim, height, width)
...
class RetinexCrossFormer_0328(nn.Module):
    ...
    def forward(self, x):
        ...
        # out_hvi = self.body(hvi) + hvi
        # out_rgb = self.trans.PHVIT(out_hvi)

        data_lowlight = x.permute(0, 2, 3, 1)
        with torch.no_grad():
